...
exports.default = WaitingApprovalPage;
...
function WaitingApprovalPage() {
    ...
    const { mutate: logout, isPending: isLoggingOut } = (0, useLogout_1.useLogout)();
    const refreshedJwt = (0, react_1.useRef)(false);
    ...
    (0, react_1.useEffect)(() => {
        ...
        (async () => {
            if (!refreshedJwt.current) {
                ...
                try {
                    await axios_1.default.post(`${env_1.env.NEXT_PUBLIC_API_URL}${api_endpoints_1.API_ENDPOINTS.AUTH.REFRESH}`, {}, { withCredentials: true });
                }
                catch {
                }
            }
            queryClient.setQueryData(auth_service_1.authKeys.session(), user);
            router.replace(routes_1.ROUTES.TEACHER.DASHBOARD);
        })();
    }, [user, router, queryClient]);
    return (<div className="bg-background text-foreground antialiased min-h-screen flex flex-col lg:flex-row transition-colors duration-200">
        ...
        <div className="w-full max-w-md space-y-8 text-center">
          ...
          <div className="flex justify-center">
            <div className="w-24 h-24 rounded-full bg-yellow-100 flex items-center justify-center shadow-lg">
              <lucide_react_1.Clock className="w-12 h-12 text-yellow-600"/>
            </div>
          </div>
          ...
          <button onClick={() => logout()} disabled={isLoggingOut} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50">
