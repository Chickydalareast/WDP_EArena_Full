...
exports.AdminBusinessDashboardScreen = AdminBusinessDashboardScreen;
...
function AdminBusinessDashboardScreen() {
    ...
    const { data, isLoading, isError } = (0, react_query_1.useQuery)({
        ...
        queryFn: () => admin_service_1.adminService.getBusinessMetrics(params),
        staleTime: 1000 * 30,
    });
    if (isLoading)
        return <div className="text-muted-foreground">Đang tải business metrics…</div>;
    if (isError || !data) {
        return (<div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6 text-destructive">
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          <div>
            ...
            <input type="date" className="mt-1 rounded-2xl border border-border bg-background/60 px-3 py-2 text-sm" value={range.from} onChange={(e) => setRange((s) => ({ ...s, from: e.target.value }))}/>
          </div>
          <div>
            <div className="text-xs font-semibold text-muted-foreground">Đến ngày</div>
            <input type="date" className="mt-1 rounded-2xl border border-border bg-background/60 px-3 py-2 text-sm" value={range.to} onChange={(e) => setRange((s) => ({ ...s, to: e.target.value }))}/>
          </div>

          <div className="flex items-end gap-2">
            <button className="rounded-2xl border border-border bg-background/60 px-3 py-2 text-sm text-foreground hover:bg-accent" onClick={() => {
            const to = new Date();
            const from = new Date();
            ...
            <button className="rounded-2xl border border-border bg-background/60 px-3 py-2 text-sm text-foreground hover:bg-accent" onClick={() => setRange({ from: '', to: '' })}>
              All-time
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard_1.StatCard label="Doanh thu (tổng)" value={`${data.revenue.total.toLocaleString()} ${data.revenue.currency}`} hint={`Đơn hàng paid: ${data.revenue.paidOrders}`} icon={<lucide_react_1.DollarSign className="size-5 text-primary"/>}/>
        ...
        <StatCard_1.StatCard label="Gợi ý monetize" value="Pro/Enterprise" hint="Cấu hình gói ở mục Gói dịch vụ" icon={<lucide_react_1.ShoppingBag className="size-5 text-primary"/>}/>
      </div>
...
//# sourceMappingURL=AdminBusinessDashboardScreen.js.map