...
exports.FreePreviewModal = FreePreviewModal;
...
const DocumentViewer = (0, dynamic_1.default)(() => import('@/shared/components/ui/document-viewer').then((mod) => mod.DocumentViewer), {
    ssr: false,
    loading: () => (<div className="flex flex-col items-center justify-center h-full w-full gap-4 bg-slate-900/5 dark:bg-black/20 min-h-[400px]">
...
function FreePreviewModal({ courseId, lesson, isOpen, onClose }) {
    const router = (0, navigation_1.useRouter)();
    const { refreshToken } = (0, useStudyRoom_1.useRefreshLessonToken)();
    const previewAccumulatedTimeRef = (0, react_1.useRef)(0);
    const [activeAttachmentId, setActiveAttachmentId] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        setActiveAttachmentId(null);
    }, [lesson?.id, isOpen]);
    const { data: lessonContent, isLoading, isError, isFetching } = (0, useStudyRoom_1.useLessonContent)(courseId, isOpen && lesson ? lesson.id : null);
    ...
    const handleStartQuiz = () => {
        if (lessonContent?.examId && lesson?.id) {
            router.push(routes_1.ROUTES.STUDENT.STUDY_ROOM(courseId, lesson.id));
            onClose();
        }
    };
    ...
    return (<dialog_1.Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <dialog_1.DialogContent className="w-[95vw] h-[90vh] max-w-none sm:max-w-5xl lg:max-w-6xl xl:max-w-7xl p-0 rounded-2xl border border-border/50 bg-background flex flex-col overflow-hidden shadow-2xl duration-300" showCloseButton={false}>
                <dialog_1.DialogDescription className="sr-only">
                    Chế độ xem trước nội dung khóa học
                </dialog_1.DialogDescription>

                <dialog_1.DialogHeader className="p-4 border-b border-border bg-card sticky top-0 z-50 flex-row items-center justify-between shrink-0 shadow-sm">
                    <dialog_1.DialogTitle className="text-lg md:text-xl font-bold flex items-center gap-3 truncate pr-4">
                    ...
                    <button_1.Button variant="ghost" size="icon" onClick={onClose} className="rounded-full hover:bg-muted shrink-0">
                        <lucide_react_1.X className="w-5 h-5"/>
                    </button_1.Button>
                </dialog_1.DialogHeader>

                <div className="flex-1 overflow-y-auto w-full bg-muted/10 relative">
                    {isLoading ? (<div className="flex flex-col items-center justify-center h-full space-y-4 min-h-[50vh]">
                        ...
                        </div>) : isError || !lessonContent ? (<div className="flex flex-col items-center justify-center h-full space-y-3 text-center p-6 min-h-[50vh]">
                            ...
                            <button_1.Button variant="outline" onClick={onClose} className="mt-4">Đóng cửa sổ</button_1.Button>
                        </div>) : (<div className="flex flex-col w-full pb-16">

                            {(hasVideo || isQuiz) && (<div className="w-full bg-black p-0 md:p-6 lg:p-8 flex justify-center shadow-inner relative">
                                    
                                    {hasVideo && lessonContent.primaryVideo?.url ? (<div className="w-full max-w-5xl mx-auto shadow-2xl rounded-xl overflow-hidden ring-1 ring-white/10">
                                            <video_player_1.VideoPlayer src={lessonContent.primaryVideo.url} accumulatedTimeRef={previewAccumulatedTimeRef} onFlush={() => { }} onTrackTimeUpdate={() => { }} onOptimisticComplete={() => { }} isRefetching={isFetching} onTokenExpired={() => {
                        if (lesson?.id)
                            refreshToken(courseId, lesson.id);
                    }}/>
                                        </div>) : isQuiz ? (<div className="flex flex-col items-center justify-center aspect-video w-full max-w-5xl mx-auto bg-slate-900 border border-slate-800 rounded-xl p-6 text-center shadow-2xl">
                                            <lucide_react_1.BrainCircuit className="w-16 h-16 text-primary mb-6"/>
                                            ...
                                            <button_1.Button size="lg" onClick={handleStartQuiz} className="font-bold text-lg px-8 h-14">
                            ...
                            <div className="px-4 md:px-8 mt-8 max-w-5xl mx-auto w-full space-y-8">
                                ...
                                {lessonContent.content && lessonContent.content !== '<p></p>' && (<div className="bg-card p-6 md:p-8 rounded-2xl border border-border shadow-sm">
                                        <h3 className="text-xl font-bold mb-6 flex items-center gap-2 border-b border-border/50 pb-4">
                                            <lucide_react_1.FileText className="w-6 h-6 text-primary"/> Ghi chú bài giảng
                                        </h3>
                                        <div className="prose prose-sm md:prose-base dark:prose-invert max-w-none leading-relaxed" dangerouslySetInnerHTML={{ __html: lessonContent.content }}/>
                                    </div>)}

                                {Array.isArray(lessonContent.attachments) && lessonContent.attachments.length > 0 && (<div className="bg-card p-6 md:p-8 rounded-2xl border border-border shadow-sm">
                                        <h3 className="text-xl font-bold mb-6 flex items-center gap-2 border-b border-border/50 pb-4">
                                            <lucide_react_1.Download className="w-6 h-6 text-orange-500"/> Tài liệu đính kèm
                                        </h3>
                        ...
                        : 'bg-background text-muted-foreground group-hover:text-orange-500 border-border/50'}`}>
                                        ...
                                        {activeAttachment && activeAttachment.url && (<div className="rounded-2xl border border-border bg-card shadow-lg overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500 ring-1 ring-primary/20">
                                                ...
                                                <div className="h-[75vh] min-h-[600px] w-full bg-slate-900/5 dark:bg-black/20">
                                                    <DocumentViewer url={activeAttachment.url} originalName={activeAttachment.originalName} isRefetching={isFetching} onTokenExpired={() => {
                        if (lesson?.id)
                            refreshToken(courseId, lesson.id);
...
//# sourceMappingURL=FreePreviewModal.js.map