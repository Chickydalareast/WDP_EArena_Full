...
exports.TeacherLessonViewer = TeacherLessonViewer;
...
function TeacherLessonViewer({ lesson }) {
    const [activeAttachmentId, setActiveAttachmentId] = (0, react_1.useState)(null);
    const dummyTimeRef = (0, react_1.useRef)(0);
    ...
    return (<div className="flex flex-col h-full bg-background pb-12 animate-in fade-in duration-500">
            ...
            {(hasVideo || isQuiz) && (<div className="w-full bg-slate-950/5 dark:bg-black p-0 md:p-6 lg:p-8 flex-shrink-0 flex flex-col gap-8 items-center justify-center border-b border-border/50" onContextMenu={(e) => e.preventDefault()}>
                    {hasVideo && lesson.primaryVideo?.url && (<div className="w-full max-w-5xl mx-auto">
                            <video_player_1.VideoPlayer src={lesson.primaryVideo.url} poster={lesson.primaryVideo.blurHash} isCompletedAtLoad={true} accumulatedTimeRef={dummyTimeRef} onFlush={() => { }} onTrackTimeUpdate={() => { }} onOptimisticComplete={() => { }}/>
                        </div>)}
                    ...
                    {isQuiz && (<div className="w-full max-w-5xl mx-auto shadow-sm">
                            <div className="bg-card border-2 border-border border-dashed rounded-xl p-12 text-center transition-all">
                                ...
                                <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-4 py-2 rounded-lg text-sm font-medium">
                                    <lucide_react_1.ShieldAlert className="w-4 h-4"/>
            ...
            hasVideo ? <lucide_react_1.Play className="w-4 h-4 text-blue-500"/> :
                <lucide_react_1.FileText className="w-4 h-4 text-orange-500"/>}
                            ...
                            {lesson.isFreePreview && (<span className="bg-green-100/80 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800 px-2.5 py-1 rounded text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <div className="shrink-0 w-full md:w-auto">
                        <div className="bg-primary/5 p-3.5 rounded-xl border border-primary/20 text-sm flex items-center justify-center gap-2 shadow-sm text-primary font-medium">
                            <lucide_react_1.Eye className="w-5 h-5"/> <span>Chế độ Xem trước (View Mode)</span>
                        </div>
                    </div>
                </div>

                {lesson.content && lesson.content !== '<p></p>' && (<div className="pt-6 border-t border-border/40">
                        <h3 className="text-lg font-bold mb-4 text-foreground">Nội dung bài học</h3>
                        <div className="prose prose-sm md:prose-base dark:prose-invert max-w-none bg-card p-6 rounded-2xl border border-border/40 shadow-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: lesson.content }}/>
                    </div>)}

                {lesson.attachments && lesson.attachments.length > 0 && (<div className="pt-6 border-t border-border/40">
                        <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                            <lucide_react_1.Download className="w-5 h-5 text-orange-500"/> Tài liệu đính kèm (PDF/DOCX)
                        </h3>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                            {lesson.attachments.map((file) => (<button key={file.id} onClick={() => setActiveAttachmentId(file.id)} className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all group text-left outline-none focus-visible:ring-2 ring-primary ${activeAttachmentId === file.id
...
//# sourceMappingURL=TeacherLessonViewer.js.map