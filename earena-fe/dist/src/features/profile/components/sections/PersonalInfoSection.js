...
function PersonalInfoSection() {
    ...
    const onSubmit = (data) => {
        ...
        updateProfile({ payload: cleanPayload, avatarFile: null }, {
            onSuccess: () => {
    ...
    return (<section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-primary/5 overflow-hidden transition-all duration-300">
            <div className="px-6 py-4 border-b border-primary/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <lucide_react_1.UserCircle className="text-primary" size={24}/>
                    <h3 className="font-bold text-lg">Thông tin cá nhân</h3>
                </div>
                {!isEditing && (<button onClick={() => setIsEditing(true)} className="text-primary text-sm font-bold flex items-center gap-1 hover:underline transition-all">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input type="text" {...register('fullName')} disabled={!isEditing || isPending} className={inputStyles}/>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input type="date" {...register('dateOfBirth')} disabled={!isEditing || isPending} className={inputStyles}/>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input type="text" {...register('phone')} disabled={!isEditing || isPending} className={inputStyles}/>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="relative flex items-center">
                            <input type="email" value={user.email || 'Chưa cập nhật'} disabled className="form-input rounded-lg border-transparent bg-transparent focus:border-transparent focus:ring-0 w-full px-0 py-0 text-slate-900 dark:text-slate-100 font-medium cursor-not-allowed pr-28"/>
                            <div className="absolute right-0 flex items-center gap-1 text-green-500 text-xs font-medium">
                                <lucide_react_1.CheckCircle2 size={16}/>
                                Đã xác minh
                            </div>
                        </div>
                    </div>
                ...
                {isEditing && (<div className="flex items-center justify-end gap-4 mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 animate-in slide-in-from-bottom-2 fade-in duration-300">
                        <button type="button" onClick={handleCancel} disabled={isPending} className="px-6 py-2 rounded-xl text-slate-500 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                            Hủy bỏ
                        </button>
                        <button type="submit" disabled={isPending} className="px-8 py-2 rounded-xl bg-primary text-white font-bold shadow-lg shadow-primary/30 hover:bg-primary/90 transition-all disabled:opacity-50">
...
//# sourceMappingURL=PersonalInfoSection.js.map