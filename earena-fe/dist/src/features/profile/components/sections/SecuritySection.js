...
function SecuritySection() {
    const [isChangingPassword, setIsChangingPassword] = (0, react_1.useState)(false);
    ...
    return (<section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-primary/5 overflow-hidden">
            
            <div className="px-6 py-4 border-b border-primary/5 flex items-center gap-2">
                <lucide_react_1.ShieldCheck className="text-primary" size={24}/>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-lg bg-slate-50 dark:bg-slate-900/50 transition-all">
                    <div className="space-y-1">
                        ...
                        <p className="font-semibold flex items-center gap-2 text-slate-900 dark:text-slate-100">
                            <lucide_react_1.Key className="text-slate-400" size={16}/>
                            Mật khẩu
                        </p>
                    </div>
                    
                    {!isChangingPassword && (<button onClick={() => setIsChangingPassword(true)} className="px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-primary/20 text-primary text-sm font-bold hover:bg-primary/5 transition-colors">
                            Đổi mật khẩu
                        </button>)}
                </div>

                
                {isChangingPassword && (<form onSubmit={handleSubmit(onSubmit)} className="mt-4 p-5 rounded-lg border border-primary/10 bg-white dark:bg-slate-800 animate-in fade-in slide-in-from-top-4 duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-1.5 md:col-span-2">
                                ...
                                <input type="password" placeholder="••••••••" {...register('oldPassword')} disabled={isPending} className="form-input rounded-lg border-primary/20 bg-slate-50 dark:bg-slate-900 focus:border-primary focus:ring-primary w-full px-4 py-2 text-slate-900 dark:text-slate-100"/>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input type="password" placeholder="••••••••" {...register('newPassword')} disabled={isPending} className="form-input rounded-lg border-primary/20 bg-slate-50 dark:bg-slate-900 focus:border-primary focus:ring-primary w-full px-4 py-2 text-slate-900 dark:text-slate-100"/>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <input type="password" placeholder="••••••••" {...register('confirmPassword')} disabled={isPending} className="form-input rounded-lg border-primary/20 bg-slate-50 dark:bg-slate-900 focus:border-primary focus:ring-primary w-full px-4 py-2 text-slate-900 dark:text-slate-100"/>
                        ...
                        <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                            <button type="button" onClick={handleCancel} disabled={isPending} className="px-5 py-2 rounded-xl text-slate-500 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                Hủy
                            </button>
                            <button type="submit" disabled={isPending} className="px-6 py-2 rounded-xl bg-primary text-white font-bold shadow-lg shadow-primary/30 hover:bg-primary/90 transition-all disabled:opacity-50">
