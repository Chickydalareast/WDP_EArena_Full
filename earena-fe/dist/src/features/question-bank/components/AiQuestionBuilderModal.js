...
exports.AiQuestionBuilderModal = AiQuestionBuilderModal;
...
function AiQuestionBuilderModal({ folderId, isOpen, onClose }) {
    const { mutate: generateAiQuestions, isPending } = (0, useBankMutations_1.useGenerateAiQuestions)();
    const { handleSubmit, setValue, watch, formState: { errors }, reset } = (0, react_hook_form_1.useForm)({
        resolver: (0, rhf_zod_resolver_1.rhfZodResolver)(question_bank_schema_1.AiQuestionBuilderSchema),
        defaultValues: {
            files: [],
            folderId: folderId,
        ...
        e.preventDefault(); }}>
                <dialog_1.DialogHeader>
                    <dialog_1.DialogTitle className="flex items-center gap-2 text-xl">
                        <lucide_react_1.Sparkles className="w-5 h-5 text-purple-600"/> AI Question Builder
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 py-4">
                    ...
                    <div className="space-y-3">
                        ...
                        <input type="file" id="ai-question-file-upload" multiple accept=".pdf,application/pdf,.docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.txt,text/plain" className="hidden" onChange={handleFileChange} disabled={isPending || selectedFiles.length >= 5}/>

                        <button_1.Button type="button" variant="outline" className="w-full h-12 border-dashed border-2 hover:bg-slate-50" onClick={() => document.getElementById('ai-question-file-upload')?.click()} disabled={isPending || selectedFiles.length >= 5}>
                            <lucide_react_1.UploadCloud className="w-4 h-4 mr-2"/> Chọn tài liệu tải lên
                        </button_1.Button>
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea_1.Textarea placeholder='VD: "Nếu câu hỏi tiếng Anh, hãy dịch nội dung sang tiếng Việt", hoặc "Tự động phân loại độ khó giúp tôi"' rows={3} onChange={(e) => setValue('additionalInstructions', e.target.value, { shouldValidate: true })} disabled={isPending}/>
                        {errors.additionalInstructions && <p className="text-[12px] font-medium text-red-500">{errors.additionalInstructions.message}</p>}
                    </div>

                    
                    {isPending && (<p className="text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-200 animate-pulse">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-border">
                        ...
                        <button_1.Button type="submit" disabled={isPending} className={isPending ? "bg-purple-600 w-[240px]" : "bg-purple-600 hover:bg-purple-700"}>
                            {isPending ? (<>
                                    ...
                                    <lucide_react_1.Sparkles className="mr-2 h-4 w-4"/> Bóc tách Đề thi
                                </>)}
                        </button_1.Button>
                    </div>
                </form>
...
//# sourceMappingURL=AiQuestionBuilderModal.js.map