...
export const adminService = {
 ...
 listCourses: async (params: { 
    ...
    return axiosClient.get(API_ENDPOINTS.ADMIN.COURSES_BASE, { params });
  },

  listPendingCourses: async (params: { page?: number; limit?: number }): Promise<PageResult<AdminCourseApproval>> => {
  ...
  approveCourse: async (id: string): Promise<void> => {
    return axiosClient.patch(API_ENDPOINTS.ADMIN.COURSE_APPROVE(id), {});
  },

  rejectCourse: async (id: string, payload: RejectCoursePayload): Promise<void> => {
    return axiosClient.patch(API_ENDPOINTS.ADMIN.COURSE_REJECT(id), payload);
  },
