...
import { Search, CheckCircle, XCircle, Eye, Loader2, ExternalLink } from 'lucide-react';
import { adminService } from '../api/admin.service';
import type { AdminTeacherVerification, TeacherVerificationStatus } from '../types/admin.types';
...
export function AdminTeacherVerificationScreen() {
  ...
  const [status, setStatus] = useState<TeacherVerificationStatus | 'PENDING'>('PENDING');
  const [selectedTeacher, setSelectedTeacher] = useState<AdminTeacherVerification | null>(null);
  ...
  const updateVerificationMut = useMutation({
  ...
  const handleApprove = (teacher: AdminTeacherVerification) => {
  ...
  const rows = (data?.items || []).map((teacher) => ({
    ...
    qualifications: (
      <div className="flex items-center gap-2">
        ...
        {teacher.hasUploadedQualifications && (
          <span className="text-xs px-2 py-0.5 rounded bg-green-50 text-green-600">Đã upload</span>
        )}
      </div>
    ),
    actions: (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <select
          className="bg-background/60 border border-border rounded-xl px-3 py-2 text-sm"
          value={status}
          onChange={(e) => setStatus(e.target.value as TeacherVerificationStatus | 'PENDING')}
        >
      ...
      <DataTable
        columns={[...TEACHER_VERIFICATION_COLUMNS]}
        rows={rows}
        isLoading={isLoading}
              ...
              <div className="border-t pt-4">
                ...
                {selectedTeacher.qualifications && selectedTeacher.qualifications.length > 0 ? (
                  <div className="space-y-2">
                    {selectedTeacher.qualifications.map((q, index) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        {q.url && (
                          <button
                            type="button"
                            onClick={() => setLightboxImage(q.url)}
                            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border text-sm text-primary hover:bg-primary/5 transition-colors"
                          >
                        ...
                        {q.url && (
                          <a
                            ...
                            className="flex items-center gap-1 px-3 py-1.5 rounded-lg border text-sm text-muted-foreground hover:text-foreground transition-colors"
                          >
                  ...
                  <p className="text-muted-foreground text-sm">Chưa upload bằng cấp nào</p>
                )}
              </div>

              {selectedTeacher.teacherVerificationStatus === 'PENDING' && (
                <div className="border-t pt-4 space-y-3">
                  <h4 className="font-medium">Ghi chú (khi từ chối)</h4>
                  <textarea
                    className="w-full min-h-[80px] p-3 rounded-lg border bg-background"
                    placeholder="Nhập lý do từ chối (sẽ được gửi qua email cho giáo viên, có thể bỏ trống)..."
                    value={rejectNote}
                  ...
                  <div className="flex gap-3">
                    <Button
                      ...
                      disabled={updateVerificationMut.isPending}
                    >
                      <CheckCircle className="w-4 h-4 mr-2" /> Duyệt tài khoản
                    </Button>
                    <Button
                      variant="destructive"
                      className="flex-1"
        ...
        <DialogContent className="max-w-4xl p-0 border-0 bg-transparent shadow-none">
          ...
          <div className="flex justify-center mt-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-background/90 text-foreground text-sm font-medium hover:bg-background transition-colors"
            >
              <ExternalLink className="w-4 h-4" />
              Mở trong tab mới
            </a>
