...
import type { Role, UserStatus } from '../types/admin.types';
...
export function AdminUsersScreen() {
  ...
  const params = useMemo(
    () => ({ page, limit: 20, search: search || undefined, role: role || undefined, status: status || undefined }),
  ...
  const setRoleMut = useMutation({
    ...
    onError: (e: any) => toast.error('Cập nhật role thất bại', { description: e?.message }),
  });
  ...
  const setStatusMut = useMutation({
    ...
    onError: (e: any) => toast.error('Cập nhật trạng thái thất bại', { description: e?.message }),
  });

  const resetPassMut = useMutation({
    mutationFn: ({ id }: { id: string }) => adminService.resetUserPassword(id),
    onSuccess: (res: any) => {
      toast.success('Reset mật khẩu thành công', {
        description: `Mật khẩu mới: ${res?.newPassword || res?.data?.newPassword || ''}`,
      });
    },
    onError: (e: any) => toast.error('Reset mật khẩu thất bại', { description: e?.message }),
  });
  ...
  const createMut = useMutation({
    ...
    onError: (e: any) => toast.error('Tạo user thất bại', { description: e?.message }),
  });
  ...
  const onQuickCreate = () => {
    ...
    const role = (window.prompt('Role (STUDENT | TEACHER | ADMIN):', 'TEACHER') || 'TEACHER') as Role;
  ...
  const rows = (data?.items || []).map((u) => ({
    ...
    role: (
      <select
        className="bg-background/60 border border-border rounded-xl px-2 py-1 text-xs"
        defaultValue={u.role}
        onChange={(e) => setRoleMut.mutate({ id: u._id, role: e.target.value as Role })}
      >
        {roles.map((r) => (
    ...
    status: (
      <select
        ...
        onChange={(e) => setStatusMut.mutate({ id: u._id, status: e.target.value as UserStatus })}
      >
        {statuses.map((s) => (
    ...
    actions: (
      <div className="flex items-center gap-2">
        <button
          className="rounded-xl border border-primary/20 bg-primary/10 px-3 py-1.5 text-xs text-primary hover:bg-primary/15"
          onClick={() => resetPassMut.mutate({ id: u._id })}
          title="Reset mật khẩu"
        >
          <RefreshCcw className="inline size-3 mr-1" /> Reset
        </button>
      </div>
    ),
  }));
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:justify-between">
        ...
        >
          <UserPlus className="size-4" /> Tạo user
        </button>
      </div>
      ...
      <DataTable
        ...
        rows={rows}
        empty={isLoading ? 'Đang tải…' : 'Chưa có user'}
      />
