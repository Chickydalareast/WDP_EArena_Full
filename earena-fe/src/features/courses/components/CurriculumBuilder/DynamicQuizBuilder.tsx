...
import { DynamicSectionItem } from './DynamicSectionItem';
...
export function DynamicQuizBuilder({ folders, topics, activeFilters, disabled }: DynamicQuizBuilderProps) {
    ...
    const { fields, append, remove } = useFieldArray({ control, name: "dynamicConfig.adHocSections" });
    ...
    return (
        <div className="space-y-6">
            ...
            <Button
                type="button"
                variant="outline"
                className="w-full py-8 border-2 border-dashed border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 hover:border-primary/40 font-bold transition-all"
                disabled={disabled}
                onClick={() => append({
                    name: `Phần ${fields.length + 1}`,
                    orderIndex: fields.length,
                    rules: [{ limit: 10, folderIds: [], topicIds: [], difficulties: [], tags: [], questionType: 'FLAT' }]
                })}
            >
                <Plus className="w-5 h-5 mr-2 text-primary/70" /> THÊM PHẦN THI MỚI
            </Button>
        </div>
    );
}