...
import { AiAutoTagSummaryModal } from './AiAutoTagSummaryModal'; // <-- [NEW] Import component
...
import { Search, FolderSync, Inbox, PlusCircle, Trash2, Copy, Loader2, Sparkles, Waypoints, RefreshCw } from 'lucide-react';
import { toast } from 'sonner';

// [NEW] Import để lấy cây chủ đề phục vụ map tên Topic
...
export function QuestionList() {
  ...
  // [NEW] Lấy data ảo đã được tích lũy
  ...
  const { 
    data: response, isLoading, isError, refetch, isRefetching 
  } = useBankQuestions({
    folderId: selectedFolderId || undefined, search: debouncedSearch, page, limit: 10,
  }, isPollingActive);

  // --- TOPIC MAPPING (O(1) Lookup) ---
  ...
  const typedQuestions = useMemo(() => {
    return questions as PopulatedQuestion[];
  }, [questions]);
  
  // Khởi động Fallback Sync Engine
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-4 border-b border-slate-200 flex flex-col lg:flex-row gap-4 justify-between items-center bg-white sticky top-0 z-30 shadow-sm">
        <div className="flex gap-2 w-full lg:w-72 shrink-0">
          ...
          <Button 
              ...
              className="shrink-0 bg-white shadow-sm" title="Làm mới dữ liệu"
          >
             <RefreshCw className={cn("w-4 h-4 text-slate-600", isRefetching && "animate-spin text-blue-500")} />
          </Button>
        </div>
          ...
          {selectedQuestionIds.length > 0 ? (
            <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-4 duration-300">
              <span className={cn(
                  ...
                  isOverLimit ? "bg-red-50 text-red-700 border-red-200" : "bg-blue-50 text-blue-700 border-blue-200"
              )}>
                Đã chọn {selectedQuestionIds.length} câu
              </span>
              
              <Button 
                  onClick={handleBulkAutoTag} disabled={isAutoTagging || isOverLimit}
                  className="bg-purple-600 hover:bg-purple-700 text-white shadow-sm font-bold"
                  title={isOverLimit ? "Chỉ hỗ trợ tối đa 200 câu hỏi mỗi lần" : "Sử dụng AI tự động đọc hiểu và gán thuộc tính"}
              >
              ...
              <Button onClick={() => setIsSmartOrganizeOpen(true)} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm">
                  <Waypoints className="w-4 h-4 mr-2" /> Sắp xếp Tự động
              </Button>

              <Button onClick={() => cloneBulk({ questionIds: selectedQuestionIds, destFolderId: selectedFolderId }, { onSuccess: () => clearSelection() })} disabled={isCloningBulk} variant="outline" className="border-amber-200 text-amber-700 hover:bg-amber-50">
                {isCloningBulk ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Copy className="w-4 h-4 mr-2" />} Nhân bản
              </Button>
              <Button onClick={() => setIsBulkMoveOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm">
      ...
      {/* 2. HEADER BẢNG */}
      <div className="px-6 py-3 border-b bg-slate-50 flex items-center justify-between text-sm text-slate-600 font-medium shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50/50 scrollbar-thin scrollbar-thumb-slate-300 relative">
        ...
        {isLoading ? (
          ...
          <div className="text-center text-red-500 py-10">Đã xảy ra lỗi khi tải dữ liệu từ máy chủ.</div>
        ) : questions.length === 0 ? (
          <div className="text-center py-24 text-slate-400">
             <Inbox className="w-16 h-16 mx-auto mb-4 opacity-20" />
             <p className="text-lg">Thư mục này chưa có câu hỏi nào.</p>
             <p className="text-sm mt-1">Bấm "Tạo câu hỏi" để bắt đầu xây dựng ngân hàng.</p>
          </div>
        ) : (
          <div className="space-y-5 pb-16">
            {questions.map((q: any) => {
              const qId = q._id || q.id;
              // [NEW] Trích xuất dữ liệu ảo từ State nếu câu này vừa được AI xử lý xong
              ...
              return (
                  <QuestionCard 
                    ...
                    optimisticData={optimisticData}   // Bơm Optimistic Data
                    topicsMap={topicsMap}             // Bơm Từ điển chuyên đề
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="text-red-600">Cảnh báo Xóa dữ liệu</DialogTitle>
            <DialogDescription>
