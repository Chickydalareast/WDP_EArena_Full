...
export class GoogleDriveAdapter implements IVideoProvider, OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    const refreshToken = this.configService.get<string>(
      'PROVIDER_GOOGLE_REFRESH_TOKEN',
    );

    this.folderId =
      this.configService.get<string>('PROVIDER_GOOGLE_DRIVE_FOLDER_ID') || '';
    ...
    if (!envFrontendUrl) {
      this.logger.warn(
        `[CORS Warning] Không tìm thấy biến FRONTEND_URL trong .env. Đang dùng fallback: ${this.frontendOrigin}. Hãy cẩn thận nếu chạy trên Production!`,
      );
    }

    if (!clientId || !clientSecret || !refreshToken) {
      this.logger.error(
        '[Infra Config] ❌ Thiếu biến môi trường PROVIDER_GOOGLE_... cho Google Auth!',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.error(
        `[Infra Error] ❌ Khởi tạo Google Drive API thất bại: ${errorMessage}`,
      );
    }
  }
  ...
  async createResumableUploadSession(fileMetadata: {
    ...
    if (!this.drive) {
      this.logger.error(
        '[GoogleDriveAdapter] Yêu cầu sinh vé upload nhưng Drive API chưa sẵn sàng.',
      );
      throw new InternalServerErrorException(
        'Dịch vụ lưu trữ Google Drive hiện không khả dụng.',
      );
    }
    ...
    try {
      ...
      if (!accessToken)
        throw new Error(
          'Không lấy được Access Token từ Google OAuth2 để cấp vé Resumable.',
        );
      ...
      if (!uploadUrlRes.ok) {
        throw new Error(
          `Resumable session API failed with status: ${uploadUrlRes.status}`,
        );
      }
      ...
      if (!uploadUrl)
        throw new Error(
          'Response từ Google không chứa Header Location (Upload URL).',
        );
      ...
      if (error instanceof GaxiosError) {
        ...
        if (errMsg === 'invalid_grant') {
          this.logger.error(
            `[CRITICAL INFRA ERROR] Google Refresh Token đã hết hạn/bị thu hồi (invalid_grant). Cập nhật .env ngay!`,
          );
          throw new InternalServerErrorException(
            'Lỗi hệ thống: Xác thực đám mây thất bại. Vui lòng báo cáo Admin.',
          );
        }

        this.logger.error(
          `[Google API Error] Lỗi sinh vé Drive: ${errMsg} - Details: ${JSON.stringify(errData)}`,
        );
      } else if (error instanceof Error) {
        this.logger.error(
          `[GoogleDriveAdapter] Lỗi xử lý luồng sinh vé: ${error.message}`,
        );
      } else {
        this.logger.error(
          `[GoogleDriveAdapter] Lỗi hệ thống không xác định: ${JSON.stringify(error)}`,
        );
      }

      throw new InternalServerErrorException(
        'Không thể khởi tạo phiên tải video do lỗi từ dịch vụ lưu trữ đám mây.',
      );
    }
  }
  ...
  async verifyUpload(fileId: string): Promise<StorageMetadata> {
    ...
    try {
      ...
      this.logger.error(
        `[GoogleDrive] Lỗi lấy metadata Video [${fileId}]: ${error.message}`,
      );
      throw new BadRequestException(
        'Không thể lấy metadata video. Có thể file chưa được Google xử lý xong hoặc file không tồn tại.',
      );
    }
  }
  ...
  async deleteVideo(fileId: string): Promise<boolean> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.warn(
        `[GoogleDriveAdapter] Cảnh báo: Không thể xóa file [${fileId}] trên Drive: ${errorMessage}`,
      );
      return false;
    }
  }
