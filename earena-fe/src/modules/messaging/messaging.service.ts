...
export class MessagingService {
  ...
  private async assertAllowedPair(aId: string, bId: string) {
    ...
    const allowed =
      (roles.has(UserRole.STUDENT) && roles.size === 1) ||
      (roles.has(UserRole.STUDENT) && roles.has(UserRole.TEACHER));

    if (!allowed) {
      throw new ForbiddenException(
        'Chỉ hỗ trợ chat giữa học sinh với học sinh hoặc giữa học sinh với giáo viên.',
      );
    }
  }
  ...
  async sendMessage(
    ...
    const peerId =
      thread.userLowId.toString() === userId
    ...
    try {
      ...
      this.logger.warn(
        `[Chat realtime] Redis publish failed: ${(err as Error)?.message}`,
      );
    }
  ...
  private async serializeChatMessageForClient(m: ChatMessageDocument) {
    let shareCourse: {
      id: string;
      title: string;
      slug: string;
