...
} from '../../exams/constants/exam-event.constant';
...
export class ExamNotificationListener {
  ...
  async handleExamSubmitted(payload: ExamSubmittedEventPayload) {
    try {
      const submission = await this.examSubmissionsRepo.findByIdSafe(
      ...
      this.logger.error(
        `[Listener Error] EXAM_SUBMITTED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleExamGraded(payload: ExamGradedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] EXAM_GRADED: ${error.message}`,
        error.stack,
      );
    }
  }
