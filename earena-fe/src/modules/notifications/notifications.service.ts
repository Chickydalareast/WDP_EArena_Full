...
export class NotificationsService implements OnModuleInit, OnModuleDestroy {
  ...
  /** Pub/sub cho chat realtime (fan-out theo userId, không lưu DB). */
  private readonly CHAT_CHANNEL = 'earena_chat';
      ...
      this.redisSubscriber.subscribe(
        ...
        (err, count) => {
          if (err) {
            ...
            this.logger.log(
              `[SSE] Redis subscriber: ${this.REDIS_CHANNEL}, ${this.CHAT_CHANNEL} (Count: ${count})`,
            );
          }
        },
      );
  ...
  /**
   * Đẩy tin nhắn chat tới SSE của user (cùng cơ chế scale ngang như notification).
   */
  private dispatchChatToClient(envelope: {
    receiverId: string;
    threadId: string;
    message: unknown;
  }) {
  ...
  /** Publish chat (gọi từ MessagingService sau khi đã commit DB). */
