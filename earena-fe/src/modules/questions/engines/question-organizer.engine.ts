...
export class QuestionOrganizerEngine {
  ...
  async generateBlueprint(
    ownerId: string,
    payload: PreviewOrganizePayload,
    ...
    if (!user || !user.subjectIds || user.subjectIds.length === 0) {
      throw new ForbiddenException(
        'Tài khoản chưa được gán Môn học, không thể dùng tính năng tự động sắp xếp theo Chuyên đề.',
      );
    }
    ...
    if (questions.length === 0)
      throw new BadRequestException('Không tìm thấy câu hỏi hợp lệ.');
    if (questions.some((q) => q.ownerId.toString() !== ownerId)) {
      throw new ForbiddenException(
        'Bạn không có quyền thao tác trên câu hỏi của người khác.',
      );
    }
    ...
    if (payload.baseFolderId) {
      const base = existingFolders.find(
        (f) => f._id.toString() === payload.baseFolderId,
      );
      if (!base)
        throw new NotFoundException(
          'Thư mục gốc (baseFolderId) không tồn tại.',
        );
    ...
    for (const q of questions) {
      if (
        !q.topicId ||
        !q.difficultyLevel ||
        q.difficultyLevel === DifficultyLevel.UNKNOWN
      ) {
        unclassifiedCount++;
        continue;
      }
      ...
      if (
        payload.strategy === OrganizeStrategy.TOPIC_STRICT ||
        payload.strategy === OrganizeStrategy.TOPIC_AND_DIFFICULTY
      ) {
      ...
      if (
        payload.strategy === OrganizeStrategy.FLAT_DIFFICULTY ||
        payload.strategy === OrganizeStrategy.TOPIC_AND_DIFFICULTY
      ) {
        const diffName =
          DIFFICULTY_NAME_MAP[q.difficultyLevel] || 'Chưa phân loại';
  ...
  async execute(ownerId: string, payload: PreviewOrganizePayload) {
    ...
    if (blueprint.mappings.length === 0) {
      throw new BadRequestException(
        'Không có câu hỏi nào hợp lệ để tổ chức (Thiếu Chuyên đề hoặc Mức độ nhận thức).',
      );
    }
    ...
    return this.foldersRepo.executeInTransaction(async () => {
      ...
      this.logger.log(
        `[Auto-Organize] User ${ownerId} executed strategy ${payload.strategy}. Created ${foldersToInsert.length} folders, organized ${blueprint.mappings.length} root questions.`,
      );
