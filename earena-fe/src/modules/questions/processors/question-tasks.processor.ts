...
export class QuestionTasksProcessor extends WorkerHost {
  ...
  private async handleAutoTagging(job: Job<AutoTagJobPayload>): Promise<void> {
    ...
    this.logger.debug(
      `[Worker] Bắt đầu xử lý AI Tagging cho ${questionIds.length} câu hỏi. Subject: ${subjectId}`,
    );

    try {
      const rawTopicTree = await this.topicsService.getTreeBySubject(subjectId);
      const { flatString: topicsDict, validTopicIds } =
        this.flattenTopicTree(rawTopicTree);
      ...
      for (let i = 0; i < questionIds.length; i += this.CHUNK_SIZE) {
        ...
        await job.updateProgress(
          Math.round(((i + chunkIds.length) / questionIds.length) * 100),
        );
      }

      this.logger.log(
        ` [Worker] Hoàn tất quá trình Auto-Tag cho ${questionIds.length} câu hỏi.`,
      );
    } catch (error: any) {
  ...
  private async processChunk(
    ...
    this.logger.debug(
      `[Batch ${batchNum}] Đang lấy dữ liệu ${chunkIds.length} câu hỏi...`,
    );
    ...
    const questions = await this.questionsRepo.modelInstance
      ...
      .select(
        'content answers type difficultyLevel topicId parentPassageId tags',
      )
      .lean()
      .exec();
    ...
    const missingParentIds = questions
      .filter(
        (q) =>
          q.parentPassageId &&
          !independentQuestions.some(
            (ind) => ind._id.toString() === q.parentPassageId.toString(),
          ),
      )
    ...
    const targetModel =
      this.configService.get<string>('GROQ_TAGGING_MODEL') ||
      'llama-3.1-8b-instant';

    this.logger.debug(
      `[Batch ${batchNum}] Bắn request sang GROQ (Model: ${targetModel})...`,
    );
    ...
    const validIndependentIds = independentQuestions.map((q) =>
      q._id.toString(),
    );
    const validUpdates = this.sanitizeAiOutput(
      aiResult.taggedQuestions,
      validIndependentIds,
      validTopicIds,
    );
    ...
    if (validUpdates.length > 0) {
        ...
        for (const updateData of validUpdates) {
          ...
          const updatePayload: any = {
            $set: {
              ...
              ...(updateData.topicId && {
                topicId: new Types.ObjectId(updateData.topicId),
              }),
            },
          };
      ...
      this.logger.debug(
        `[Batch ${batchNum}] Đã lưu và Cascade thuộc tính Mẹ-Con thành công (An toàn ACID).`,
      );

      this.eventEmitter.emit(
        NotificationEventPattern.QUESTION_AUTO_TAG_BATCH_COMPLETED,
        {
          teacherId,
          batchNum,
          totalBatches,
  ...
  private flattenTopicTree(
    tree: any[],
    prefix = '',
  ): { flatString: string; validTopicIds: Set<string> } {
    let flatString = '';
    ...
    for (const node of tree) {
      ...
      if (node.children && node.children.length > 0) {
        const childResult = this.flattenTopicTree(
          node.children,
          prefix + '-- ',
        );
