    ...
    return this.transactionsRepo.executeInTransaction(async () => {
      ...
      if (!targetPlan || !targetPlan.isActive) {
        throw new NotFoundException(
          'Gói cước không tồn tại hoặc đã ngừng cung cấp.',
        );
      }
      ...
      const isCurrentActive =
        currentPlan && currentExpiresAt && currentExpiresAt > now;
      ...
      if (isCurrentActive) {
        const currentWeight =
          this.PLAN_WEIGHT[currentPlan.code as PricingPlanCode] || 0;
        const targetWeight =
          this.PLAN_WEIGHT[targetPlan.code as PricingPlanCode] || 0;

        if (targetWeight < currentWeight) {
          throw new BadRequestException(
            `Bạn đang sử dụng gói cao hơn (${currentPlan.name}). Vui lòng chờ đến khi hết hạn để đổi gói.`,
          );
          ...
          if (lastTx) {
            const lastCycleDays =
              lastTx.billingCycle === BillingCycle.YEARLY ? 365 : 30;
            dailyRate = lastTx.finalAmount / lastCycleDays;
          } else {
            const fullCurrentPlan = await this.pricingPlansRepo.findByIdSafe(
              currentPlan._id,
              { select: 'priceMonthly' },
            );
            if (!fullCurrentPlan) {
              throw new InternalServerErrorException(
                'Dữ liệu đối chiếu gói cước hiện tại bị lỗi.',
              );
            }
      ...
      if (!updatedUser) {
        throw new InternalServerErrorException(
          'Lỗi đồng bộ dữ liệu người dùng.',
        );
      }
