...
import { User, UserDocument, UserRole, UserStatus, TeacherVerificationStatus } from '../users/schemas/user.schema';
...
import { PricingPlan, PricingPlanDocument, PricingPlanCode } from './schemas/pricing-plan.schema';
import { SubscriptionTransaction, SubscriptionTransactionDocument, TransactionStatus } from './schemas/subscription-transaction.schema';
...
export class AdminService {
  constructor(
    ...
    @InjectModel(PricingPlan.name) private readonly planModel: Model<PricingPlanDocument>,
    @InjectModel(SubscriptionTransaction.name) private readonly txModel: Model<SubscriptionTransactionDocument>,
  ...
  async updateUserRole(id: string, role: UserRole) {
    const updated = await this.userModel.findByIdAndUpdate(id, { $set: { role } }, { new: true }).lean();
    if (!updated) throw new NotFoundException('User không tồn tại');
    return { success: true };
  }

  async updateUserStatus(id: string, status: UserStatus) {
    const updated = await this.userModel.findByIdAndUpdate(id, { $set: { status } }, { new: true }).lean();
    if (!updated) throw new NotFoundException('User không tồn tại');
    return { success: true };
  }
  ...
  async deactivateUser(id: string) {
    const updated = await this.userModel.findByIdAndUpdate(id, { $set: { status: UserStatus.INACTIVE } }, { new: true }).lean();
    if (!updated) throw new NotFoundException('User không tồn tại');
    return { success: true };
  }
  ...
  async setExamPublish(id: string, isPublished: boolean) {
    const updated = await this.examModel.findByIdAndUpdate(id, { $set: { isPublished } }, { new: true }).lean();
  ...
  async setClassLocked(id: string, isLocked: boolean) {
    const updated = await this.classModel.findByIdAndUpdate(id, { $set: { isLocked } }, { new: true }).lean();
  ...
  async setQuestionArchived(id: string, isArchived: boolean) {
    const updated = await this.questionModel.findByIdAndUpdate(id, { $set: { isArchived } }, { new: true }).lean();
  ...
  async updateSubject(id: string, payload: any) {
    const updated = await this.subjectModel.findByIdAndUpdate(id, { $set: payload }, { new: true }).lean();
  ...
  async updatePricingPlan(id: string, payload: any) {
    const updated = await this.planModel.findByIdAndUpdate(id, { $set: payload }, { new: true }).lean();
    if (!updated) throw new NotFoundException('Pricing plan không tồn tại');
    return updated;
  }

  // ---------------- Teacher verification ----------------
  async listTeacherVerifications(query: { page: number; limit: number; status?: TeacherVerificationStatus; search?: string }): Promise<PageResult<any>> {
    const { page, limit, status, search } = query;
    const filter: any = { role: UserRole.TEACHER };
    if (status) {
      // Backward-compatible: teacher docs created before adding the field will be treated as PENDING
      if (status === TeacherVerificationStatus.PENDING) {
        filter.teacherVerificationStatus = { $in: [TeacherVerificationStatus.PENDING, null] };
      } else {
  ...
  async updateTeacherVerification(adminId: string, teacherId: string, payload: { status: TeacherVerificationStatus; note?: string }) {
  ...
  async getBusinessMetrics(query: { from?: string; to?: string }) {
    ...
    const revenueAgg = await this.txModel.aggregate([
      { $match: match },
    ...
    return {
      ...
      note: 'Doanh thu được tổng hợp từ subscription_transactions (PAID). Nếu chưa tích hợp thanh toán, số liệu mặc định = 0.',
    };
  }
}
