...
import {
  aiMulterOptions,
  AI_UPLOAD_LIMITS,
} from '../constants/ai-upload.constant';
...
export class AiCourseBuilderController {
  ...
  @UseInterceptors(
    FilesInterceptor('files', AI_UPLOAD_LIMITS.MAX_FILES, aiMulterOptions),
  )
  async generateCourseContent(
    ...
    if (!files || files.length === 0) {
      throw new BadRequestException(
        'Bắt buộc phải tải lên ít nhất 1 tài liệu để AI phân tích.',
      );
    }
    ...
    const result =
      await this.aiCourseBuilderService.generateCurriculum(payload);
