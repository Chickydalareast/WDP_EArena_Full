import { Controller, Post, Body, UseGuards, Delete, Query, Put, Get, Param, BadRequestException } from '@nestjs/common';
...
export class CourseQuizBuilderController {
    ...
    private mapDynamicConfig(dtoConfig?: any): DynamicExamConfigParam | undefined {
        if (!dtoConfig) return undefined;

        return {
            ...(dtoConfig.sourceFolders && { sourceFolders: dtoConfig.sourceFolders.map((id: string) => new Types.ObjectId(id)) }),
            ...(dtoConfig.mixRatio && { mixRatio: dtoConfig.mixRatio }),
            ...(dtoConfig.matrixId && { matrixId: new Types.ObjectId(dtoConfig.matrixId) }),
            ...(dtoConfig.adHocSections && { adHocSections: dtoConfig.adHocSections })
        };
    }
    ...
    async createQuizLesson(
        ...
        const params: CreateCourseQuizParams = {
            ...
            dynamicConfig: this.mapDynamicConfig(dto.dynamicConfig) as DynamicExamConfigParam,
            examRules: {
