...
export class CourseProgressionListener {
  ...
  async handleLessonCompletedForReview(payload: LessonCompletedEventPayload) {
    ...
    try {
      ...
      const isExactly20Percent =
        completedLessons === Math.ceil(totalLessons * 0.2);
      ...
      let message =
        'Bạn đã đi được một chặng đường tuyệt vời. Hãy dành 1 phút chia sẻ cảm nhận để giúp khóa học tốt hơn nhé!';
      ...
      this.logger.log(
        `[SSE Bridge] Đã bắn tín hiệu ${actionType} tới User ${payload.userId} cho Course ${payload.courseId}`,
      );
    } catch (error: any) {
      this.logger.error(
        `[Progression Listener Error] Lỗi khi xử lý Milestone Review: ${error.message}`,
        error.stack,
      );
    }
  }
