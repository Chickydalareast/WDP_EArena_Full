  ...
  async checkoutCourse(payload: CourseCheckoutPayload) {
    ...
    if (course.teacherId.toString() === payload.userId)
      throw new BadRequestException(
        'Giáo viên không thể tự mua khóa học của mình.',
      );

    let revenueEarned = 0;

    const transactionResult = await this.coursesRepo.executeInTransaction(
      async () => {
        const existing = await this.enrollmentsRepo.findUserEnrollment(
          payload.userId,
          payload.courseId,
        );
        if (existing)
