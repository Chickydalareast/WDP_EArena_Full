...
import { CourseEventPattern, CourseReviewedEventPayload, ReviewRepliedEventPayload } from '../constants/course-event.constant';
...
export class CourseReviewsService {
    ...
    private async clearCourseCache(slug: string) {
        try {
            ...
            const redisClient = (this.redisService as any).redisClient || (this.redisService as any).redis;
            if (redisClient) {
    ...
    async createReview(payload: CreateCourseReviewPayload) {
        ...
        const course = await this.coursesRepo.findByIdSafe(payload.courseId, { select: 'status slug teacherId title' });
        ...
        if (!enrollment || enrollment.status !== EnrollmentStatus.ACTIVE) {
            throw new ForbiddenException('Bạn phải ghi danh khóa học này trước khi để lại đánh giá.');
        }

        if (enrollment.progress === 0) {
            throw new ForbiddenException('Bạn cần trải nghiệm ít nhất một bài học (progress > 0%) trước khi đưa ra đánh giá.');
        }
        ...
        const statsResult = await this.coursesRepo.executeInTransaction(async () => {
            try {
                ...
                if (error.code === 11000) {
                    throw new BadRequestException('Bạn đã gửi đánh giá cho khóa học này rồi. Không thể đánh giá thêm.');
                }
                throw error;
            }
    ...
    async replyReview(payload: ReplyCourseReviewPayload) {
        ...
        const review = await this.reviewsRepo.findByIdSafe(payload.reviewId, { select: 'courseId userId' });
        if (!review) {
            throw new NotFoundException('Không tìm thấy bài đánh giá này.');
        }

        // Kéo title để bắn notification
        const course = await this.coursesRepo.findByIdSafe(review.courseId, { select: 'teacherId slug title' });
        if (!course) {
            throw new NotFoundException('Khóa học liên kết với đánh giá không tồn tại.');
        }

        if (course.teacherId.toString() !== payload.teacherId) {
            throw new ForbiddenException('Bạn không có quyền phản hồi bài đánh giá của khóa học này.');
        }
