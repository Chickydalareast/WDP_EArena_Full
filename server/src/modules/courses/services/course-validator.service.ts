...
import { UsersRepository, PopulatedSubscriptionPlan } from '../../users/users.repository';
...
export class CourseValidatorService {
  ...
  async validateTeacherSubjectAccess(teacherId: string, subjectId: string): Promise<void> {
    ...
    if (!teacher || !teacher.subjectIds || teacher.subjectIds.length === 0) {
      throw new BadRequestException('Hồ sơ giáo viên chưa được cấu hình bộ môn chuyên trách.');
    }

    const hasAccess = teacher.subjectIds.some(id => id.toString() === subjectId);
    if (!hasAccess) {
      throw new ForbiddenException('Bạn không có quyền tạo khóa học cho bộ môn này.');
    }
  }

  async verifyMediaOwnershipStrict(mediaIds: (string | null | undefined)[], teacherId: string): Promise<void> {
  ...
  async verifyExamOwnershipStrict(examId: string | undefined, teacherId: string): Promise<void> {
  ...
  async validateCourseSubmissionRules(payload: ValidateCourseSubmissionPayload): Promise<void> {
    ...
    const currentPlan = teacher.currentPlanId as unknown as PopulatedSubscriptionPlan;
    const planExpiresAt = teacher.planExpiresAt;
    const now = new Date();

    if (!currentPlan) {
      throw new ForbiddenException('Tài khoản của bạn chưa đăng ký gói cước. Vui lòng nâng cấp để xuất bản khóa học.');
    }

    if (!currentPlan.isActive) {
      throw new ForbiddenException(`Gói cước "${currentPlan.name}" hiện đã ngừng cung cấp hoặc bị vô hiệu hóa. Vui lòng liên hệ Admin.`);
    }

    if (!planExpiresAt || new Date(planExpiresAt) <= now) {
      throw new ForbiddenException(`Gói cước "${currentPlan.name}" của bạn đã hết hạn. Vui lòng gia hạn để tiếp tục xuất bản khóa học.`);
    }
