...
export class LearningTrackingService implements OnModuleInit {
    ...
    private async processInstantCompletion(payload: SyncHeartbeatPayload, redisKey: string) {
        try {
            ...
            const updatedProgress = await this.progressRepo.atomicUpsertProgress(
                payload.userId, payload.courseId, payload.lessonId,
            ...
            if (updatedProgress && !updatedProgress.isCompleted) {
                ...
                if (duration > 0 && updatedProgress.watchTime >= duration * this.COMPLETION_THRESHOLD) {
                    ...
                    await this.enrollmentsService.markLessonCompleted({
                        userId: payload.userId, courseId: payload.courseId, lessonId: payload.lessonId
                    });
