...
export class ExamMatricesService {
    ...
    async createMatrixTemplate(teacherId: string, payload: CreateMatrixPayload) {
        ...
        return { message: 'Lưu Khuôn mẫu Ma trận thành công', matrixId: matrix._id };
    }
    ...
    async getMatrixDetail(matrixId: string, teacherId: string) {
        if (!Types.ObjectId.isValid(matrixId)) throw new BadRequestException('ID Ma trận không hợp lệ');

        const matrix = await this.matricesRepo.findByIdSafe(matrixId);

        if (!matrix) throw new NotFoundException('Không tìm thấy Khuôn mẫu Ma trận');
    ...
    async updateMatrix(matrixId: string, teacherId: string, payload: UpdateMatrixPayload) {
        ...
        if (payload.subjectId !== undefined) updateData.subjectId = new Types.ObjectId(payload.subjectId);
        ...
        const updated = await this.matricesRepo.updateByIdSafe(matrixId, { $set: updateData });
        this.logger.log(`[Matrix Template] Teacher ${teacherId} updated Matrix ${matrixId}`);
        return { message: 'Cập nhật Khuôn mẫu Ma trận thành công', matrixId: updated?._id };
    }
    ...
    async cloneMatrix(matrixId: string, teacherId: string): Promise<CloneMatrixResult> {
        ...
        if (!source) throw new NotFoundException('Không tìm thấy Khuôn mẫu Ma trận nguồn');
