...
import { MediaContext, MediaDocument, MediaProvider, MediaStatus } from './schemas/media.schema';
import type {
  ICloudinaryProvider,
  IVideoProvider,
  StorageMetadata,
...
export class MediaService {
  ...
  private readonly SUPPORTED_IMAGE_FORMATS = ['jpg', 'jpeg', 'png', 'webp', 'heic'];
  private readonly SUPPORTED_VIDEO_FORMATS = ['mp4', 'webm', 'mkv', 'mov', 'avi'];
  private readonly SUPPORTED_AUDIO_FORMATS = ['mp3', 'wav', 'ogg', 'm4a', 'mpeg'];

  constructor(
    @Inject(CLOUDINARY_PROVIDER) private readonly cloudinaryProvider: ICloudinaryProvider,
  ...
  getSecureUrl(publicId: string, mimetype: string, expiresInSeconds: number = 7200): string {
  ...
  // =======================================================================
  async syncCloudinaryMedia(userId: string, payload: SyncCloudinaryPayload): Promise<MediaDocument> {
    ...
    const isVideo = this.SUPPORTED_VIDEO_FORMATS.includes(formatLower) || payload.context === MediaContext.LESSON_VIDEO;
    ...
    if (metadata.bytes > SYSTEM_MAX_BYTES || metadata.bytes === 0) {
      ...
      await this.cloudinaryProvider.deleteFile(metadata.publicId, resourceType).catch(() => null);
      throw new BadRequestException('Kích thước file không hợp lệ hoặc vượt giới hạn hệ thống.');
    }
    ...
    if (isVideo) {
      ...
      resolvedMimetype = metadata.format === 'mp3' ? 'audio/mpeg' : `audio/${metadata.format}`;
    ...
    if (!isVideo && !isAudio && this.SUPPORTED_IMAGE_FORMATS.includes(formatLower)) {
  ...
  async requestVideoUploadTicket(userId: string, payload: RequestTicketPayload) {
  ...
  async confirmUpload(mediaId: string, userId: string): Promise<MediaDocument> {
    ...
    if (!media) throw new BadRequestException('Bản ghi tài nguyên không tồn tại');
    if (media.uploadedBy.toString() !== userId) throw new ForbiddenException('Bạn không có quyền xác nhận file này');

    if (media.status === MediaStatus.READY) return media;

    const verifier = this.uploadVerifiers.get(media.provider);
    if (!verifier) {
      throw new InternalServerErrorException(`Hệ thống chưa hỗ trợ xác nhận luồng PENDING cho nền tảng: ${media.provider}`);
    }

    try {
      const metadata = await verifier.verifyUpload(media.publicId);
      if (metadata.bytes === 0) throw new BadRequestException('Tệp tin rỗng (0 bytes).');
      ...
      throw new BadRequestException(error.message || 'Không thể xác minh file trên hệ thống lưu trữ.');
    }
  }

  // Luồng upload qua server cũ (Đã được controller khóa mồm nhưng service vẫn giữ để FE không sập chùm)
  async uploadSingleFromFile(file: Express.Multer.File, userId: string, context: MediaContext): Promise<any> {
    // Tạm để nguyên code cũ ở đây nếu hệ thống FE vẫn đang còn dùng.
    // Nếu rảnh, tôi sẽ xóa nó ở Phase dọn rác.
    throw new BadRequestException('Hàm upload file đồng bộ đã bị khóa. Vui lòng dùng luồng Signature mới.');
  }
}