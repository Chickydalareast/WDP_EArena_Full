...
export class CourseNotificationListener {
  ...
  async handleCourseCompleted(payload: CourseCompletedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_COMPLETED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCourseApproved(payload: CourseApprovedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_APPROVED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCourseRejected(payload: CourseRejectedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_REJECTED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCourseReviewed(payload: CourseReviewedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_REVIEWED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleReviewReplied(payload: ReviewRepliedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] REVIEW_REPLIED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCoursePurchased(payload: CoursePurchasedEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_PURCHASED: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCourseSold(payload: CourseSoldEventPayload) {
    try {
      ...
      this.logger.error(
        `[Listener Error] COURSE_SOLD: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async handleCourseNewLesson(payload: CourseNewLessonEventPayload) {
    try {
      const studentIds =
        await this.enrollmentsRepo.findActiveStudentIdsByCourse(
          payload.courseId,
        );
