...
export class WalletNotificationListener {
    ...
    async handleWithdrawalRequested(payload: WithdrawalRequestedEventPayload) {
        try {
            const teacher = await this.usersRepo.findByIdSafe(payload.teacherId, { select: 'fullName' });
            ...
            // Tìm Admin
            const admins = await this.usersRepo.modelInstance.find({ role: UserRole.ADMIN }).select('_id').lean().exec();
            ...
            await Promise.all(
                admins.map(admin =>
                    this.notificationsService.createAndDispatch({
                        ...
                        payload: { requestId: payload.requestId, url: '/admin/withdrawals' }
                    })
                )
            );
        } catch (error: any) {
