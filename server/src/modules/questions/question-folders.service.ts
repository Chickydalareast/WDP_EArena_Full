import { ConflictException, ForbiddenException, Injectable, Logger, BadRequestException, NotFoundException } from '@nestjs/common';
...
export class QuestionFoldersService {
  ...
  async createFolder(ownerId: string, payload: CreateFolderPayload) {
    ...
    if (payload.parentId) {
      if (!Types.ObjectId.isValid(payload.parentId)) throw new BadRequestException('ID thư mục gốc không hợp lệ.');
      const parent = await this.foldersRepo.findByIdSafe(new Types.ObjectId(payload.parentId));

      if (!parent || parent.ownerId.toString() !== ownerId) {
        throw new ForbiddenException('Thư mục gốc không tồn tại hoặc bạn không có quyền truy cập.');
      }
      parentAncestors = [...(parent.ancestors || []), parent._id as Types.ObjectId];
    }
  ...
  async updateFolder(folderId: string, ownerId: string, payload: UpdateFolderPayload) {
    if (!Types.ObjectId.isValid(folderId)) throw new BadRequestException('ID thư mục không hợp lệ.');
    ...
    return this.foldersRepo.executeInTransaction(async () => {
      ...
      if (payload.parentId !== undefined && payload.parentId !== folder.parentId?.toString()) {
        ...
        if (payload.parentId === null) {
          ...
          if (!Types.ObjectId.isValid(payload.parentId)) throw new BadRequestException('ID thư mục cha không hợp lệ.');

          if (payload.parentId === folderId) throw new BadRequestException('Thư mục cha không thể là chính nó.');

          const newParent = await this.foldersRepo.findByIdSafe(new Types.ObjectId(payload.parentId));
          if (!newParent || newParent.ownerId.toString() !== ownerId) {
            throw new ForbiddenException('Thư mục cha không tồn tại hoặc bạn không có quyền truy cập.');
          }

          const isTargetInsideCurrent = newParent.ancestors.some(a => a.toString() === folderId);
          if (isTargetInsideCurrent) {
            throw new ConflictException('Không thể di chuyển thư mục cha vào bên trong thư mục con của chính nó.');
          }
        ...
        if (descendants.length > 0) {
          const bulkOps = descendants.map(desc => {
            ...
            const rebuiltAncestors = [...newAncestors, folderObjectId, ...relativeAncestors];
          ...
          await this.foldersRepo.modelInstance.bulkWrite(bulkOps, { session: (this.foldersRepo as any).currentSession });
        }
      } else {
  ...
  async deleteFolder(folderId: string, ownerId: string) {
    ...
    const hasChildFolders = await this.foldersRepo.modelInstance.exists({ ancestors: folderObjectId });
    if (hasChildFolders) {
      throw new ConflictException('Không thể xóa! Thư mục này đang chứa các thư mục con bên trong.');
    }

    const hasQuestions = await this.questionsRepo.modelInstance.exists({ folderId: folderObjectId });
    if (hasQuestions) {
      throw new ConflictException('Không thể xóa! Thư mục này đang chứa câu hỏi. Vui lòng di chuyển hoặc xóa hết câu hỏi trước.');
    }
