...
import { TransactionType, ReferenceType } from './schemas/wallet-transaction.schema';
import {
  MockDepositPayload,
...
export class WalletsService {
  ...
  async getMyTransactions(payload: GetTransactionsPayload) {
    ...
    return this.walletTransactionsRepo.getMyTransactions(wallet._id as Types.ObjectId, payload.page, payload.limit);
  }

  async mockDeposit(payload: MockDepositPayload) {
    if (payload.amount <= 0) throw new BadRequestException('Số tiền nạp phải lớn hơn 0.');

    return this.walletsRepo.executeInTransaction(async () => {
      const wallet = await this.walletsRepo.getOrCreateWallet(payload.userId);
      const updatedWallet = await this.walletsRepo.atomicAdd(wallet._id as Types.ObjectId, payload.amount);

      if (!updatedWallet) throw new InternalServerErrorException('Lỗi hệ thống khi nạp tiền.');

      await this.walletTransactionsRepo.createDocument({
        walletId: updatedWallet._id as Types.ObjectId,
  ...
  async processPayment(payload: ProcessPaymentPayload) {
    ...
    const updatedWallet = await this.walletsRepo.atomicDeduct(wallet._id as Types.ObjectId, payload.amount);

    if (!updatedWallet) {
      throw new BadRequestException('Số dư ví không đủ để thực hiện giao dịch. Vui lòng nạp thêm Coin.');
    }
  ...
  async processSplitPayment(payload: ProcessSplitPaymentPayload) {
    ...
    const updatedBuyerWallet = await this.walletsRepo.atomicDeduct(buyerWallet._id as Types.ObjectId, payload.amount);

    if (!updatedBuyerWallet) {
      throw new BadRequestException('Số dư ví không đủ để thanh toán khóa học. Vui lòng nạp thêm Coin.');
    }

    await this.walletTransactionsRepo.createDocument({
      walletId: updatedBuyerWallet._id as Types.ObjectId,
      type: TransactionType.PAYMENT,
      amount: payload.amount,
    ...
    if (revenueAmount > 0) {
      ...
      const updatedSellerWallet = await this.walletsRepo.atomicAdd(sellerWallet._id as Types.ObjectId, revenueAmount);

      if (!updatedSellerWallet) {
        this.logger.error(`[CRITICAL] Lỗi cộng doanh thu ${revenueAmount} cho Giáo viên ${payload.sellerId}`);
        throw new InternalServerErrorException('Lỗi hệ thống: Không thể xử lý chia sẻ doanh thu.');
      }

      await this.walletTransactionsRepo.createDocument({
        walletId: updatedSellerWallet._id as Types.ObjectId,
  ...
  async processSubscriptionPayment(payload: ProcessSubscriptionPaymentPayload) {
    ...
    if (!updatedWallet) {
      throw new BadRequestException('Số dư ví không đủ để nâng cấp gói cước. Vui lòng nạp thêm Coin.');
    }
  ...
  async processWithdrawalRequest(payload: ProcessWithdrawalPayload) {
    const transactionResult = await this.withdrawalRequestsRepo.executeInTransaction(async () => {
      ...
      if (!request) throw new NotFoundException('Yêu cầu rút tiền không tồn tại.');

      if (request.status !== WithdrawalStatus.PENDING) {
        throw new BadRequestException(`Yêu cầu này đã được xử lý (Trạng thái hiện tại: ${request.status}).`);
      }

      const teacher = await this.usersRepo.findByIdSafe(request.teacherId, { select: 'email fullName' });
      if (!teacher) throw new InternalServerErrorException('Không tìm thấy thông tin chủ tài khoản.');
      ...
      if (payload.action === 'APPROVE') {
        ...
        if (request.transactionId) {
          await this.walletTransactionsRepo.updateByIdSafe(request.transactionId, {
            $set: { description: `Hoàn tất rút tiền ${amountFormatted} thành công.` }
          });
        }
        ...
        if (!payload.rejectionReason) throw new BadRequestException('Bắt buộc phải nhập lý do từ chối.');

        const teacherWallet = await this.walletsRepo.getOrCreateWallet(request.teacherId);

        const updatedWallet = await this.walletsRepo.atomicAdd(teacherWallet._id as Types.ObjectId, request.amount);
        if (!updatedWallet) throw new InternalServerErrorException('Lỗi hệ thống: Không thể hoàn tiền vào ví.');
    ...
    try {
      if (transactionResult.action === 'APPROVE') {
        await this.mailService.addWithdrawalApprovalJob(transactionResult.mailPayload as any);
        ...
        await this.mailService.addWithdrawalRejectionJob(transactionResult.mailPayload as any);
