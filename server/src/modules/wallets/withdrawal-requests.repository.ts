...
export class WithdrawalRequestsRepository extends AbstractRepository<WithdrawalRequestDocument> {
    ...
    constructor(
        @InjectModel(WithdrawalRequest.name) private readonly requestModel: Model<WithdrawalRequestDocument>,
        @InjectConnection() connection: Connection,
    ) {
        super(requestModel, connection);
    }

    async getAdminWithdrawalRequests(status: string | undefined, page: number, limit: number) {
        const filter: any = {};
